...
- 현재 디렉토리 및 하위 디렉토리 전체에서 파일명 검색

### find / -name [파일명]

- 전체 시스템 (루트 디렉토리부터) 내에서 파일명 검색
...
- 파일명에 해당 문자열을 포함하고 있는 파일 검색

### find -name "[문자열]\*"

- 파일명이 해당 문자열로 시작하는 파일 검색

### find -name "\*[문자열]"

- 파일명이 해당 문자열로 끝나는 파일 검색

### find -size +[n]c -and -size -[m]c

- 파일 크기가 n 이상 m 이하인 파일 검색
- c : 바이트, k : 킬로바이트, M : 메가바이트, G : 기가바이트
- ls -al 할때 표시되는 용량은 바이트 단위임

### find [파일명] -ls -l {} \;

- 검색한 파일의 상세 정보 출력

### [더 많은 find 관련 정보](https://recipes4dev.tistory.com/156#312-%ED%8C%8C%EC%9D%BC-%ED%81%AC%EA%B8%B0%EB%A5%BC-%EC%82%AC%EC%9A%A9%ED%95%98%EC%97%AC-%ED%8C%8C%EC%9D%BC-%EA%B2%80%EC%83%89)
